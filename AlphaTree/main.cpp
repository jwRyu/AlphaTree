...
// void RandomizedHDRimage(int64* hdrimg, uint8* ldrimg, int64 imgsize, int8 bit_depth)
// {
// 	uint64 pix;
// 
...
// 				}
// //				f << p.path().string().c_str() << '\t' << height << '\t' << width << '\t' << max_memuse << '\t' << 0 << '\t' << tree->maxSize << '\t' << tree->curSize << '\t' << minruntime << endl;
// 
// 				pixels_processed = width * height;
// 				time_elapsed = minruntime;
// 				cout << tree->get_maxSize() << endl;
